...
{{-- <div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-xl font-semibold">Daftar Ruangan</h2>
        </div>
        ...
        <div class="divide-y divide-gray-200">
            @foreach($rooms as $room)
                @php
                    $isBooked = in_array($room->id, $bookedRoomIds);
                @endphp
                <div class="p-6 {{ $isBooked ? 'text-gray-400' : '' }} hover:bg-gray-50 transition duration-150">
                    <div class="flex justify-between items-start">
                        <div>
                            <h3 class="text-lg font-semibold {{ $isBooked ? 'text-gray-400' : 'text-blue-600' }}">
                                {{ $room->name }}
                            </h3>
                        ...
                        <div>
                            <a href="{{ route('bookings.create', ['room_id' => $room->id]) }}" 
                            class="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md shadow-sm">
                                Pilih Ruangan
                            </a>
                        </div>
                        @endunless
